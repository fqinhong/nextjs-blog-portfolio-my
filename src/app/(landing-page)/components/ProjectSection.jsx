...
const ProjectSection = () => {
  ...
  return (
    <section id="projects" className="px-4 py-21 sm:px-6 lg:px-8">
      <h2 className="text-3xl font-bold text-white mb-12">Feature Projects</h2>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {projects.map((project, index) => (
          <button
            onClick={() => handProjectClick(project.id)}
            key={index}
            ...
            <div className="flex flex-col h-full">
              ...
              <div className="flex flex-wrap gap-1 mt-auto">
                {project.tags.map((tag, index) => (
                  <span
                    key={index}
                    className="px-2 py-0.5 text-xs rounded-full bg-primary/20 text-white border border-primary/30"
                  >
                    {tag}
                  </span>
                ))}
